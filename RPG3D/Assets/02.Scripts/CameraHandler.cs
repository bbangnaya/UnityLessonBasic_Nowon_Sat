...
public class CameraHandler : MonoBehaviour
{
    ...
    [SerializeField] private float yPointSpeed = 300; // 마우스 y 축 이동속도
    ...
    private void Start() {
        target = PlayerMove.instance.transform;             // awake에서는 다른 객체에 접근하지 않으므로 start에서 초기화
        distance = Vector3.Distance(tr.position, target.position);      // 플레이어와 카메라와의 거리
    }

    private void Update() {
        x += Input.GetAxis("Mouse X") * xPointSpeed * Time.deltaTime;
        y += Input.GetAxis("Mouse Y") * yPointSpeed * Time.deltaTime;       // 상하조정 (-=는 상하반전)
